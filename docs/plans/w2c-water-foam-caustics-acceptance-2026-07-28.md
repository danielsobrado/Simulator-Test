...
Status: implemented on `agent/w2c-water-effects`.

Dependencies: W0, W1, W3, W4, W2A, and W2B are merged on `main`.
...
The original render and prewarm methods are restored during disposal. Restoration is ownership-safe: a later subsystem replacing either hook is not overwritten.

The last valid water-surface elevation is retained while the underwater blend fades out, avoiding a post-surface jump to world height zero.
