...
- Doors and windows also support default-on **Auto-join openings** behavior. Same-kind, same-row openings that overlap or meet the repeated-spacing gap become one reversible multi-panel assembly with a shared structural void, outer frame, sill or lintel, and flower box. Joined panes or leaves retain their relative proportions, edit as one component, can be separated again, and follow planar or round attachment surfaces.
- With auto-join disabled, or when kinds/rows are incompatible, move and resize edits resolve to the nearest collision-free façade-space transform. Wall containment remains mandatory and is independent of Smart snap or its temporary Shift inversion.
