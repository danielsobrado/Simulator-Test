...
export function layoutMerlon(merlon, {
  ...
  const bridge = lane(shape, 0) < 0.5 ? 1 : 3;
  const rows = bridge === 1 ? 2 : 4;
  // "The central polygon is deleted." Only the tall variant, and not always, so a
  // crown reads as a run of solid merlons with the odd arrow loop in it.
  const pierced = bridge === 3 && lane(shape, 8) < 0.7;
  const columns = pierced ? 3 : 2;
  ...
  for (let row = 0; row < rows; row += 1) {
    ...
    const cells = pierced ? columns : columns + (row % 2);
  ...
  // off the side of the crown — a corbel, in masonry terms. Categorised as coping
  // so `IRREGULARITY_CATEGORY_SCALE` keeps it worked rather than rubbly; a spur
  // shaped like field rubble reads as a stone that fell and stuck.
  if (lane(spurHash, 0) < 0.32) {
