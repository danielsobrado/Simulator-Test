...
function createRainGeometry(seed) {
  ...
  for (let i = 0; i < DROP_COUNT; i++) {
    ...
    offset[o + 3] = rng.range(13, 27);
    shape[o] = rng.range(0.7, 1.65);
    shape[o + 1] = rng.range(8e-3, 0.022);
    shape[o + 2] = rng.float();
    shape[o + 3] = rng.float();
  }
...
function createSnowGeometry(seed) {
  const geometry = new THREE.InstancedBufferGeometry();
  geometry.setAttribute("position", new THREE.BufferAttribute(new Float32Array([
    -1,
    -1,
...
function createSandstormGeometry(seed) {
  const geometry = new THREE.InstancedBufferGeometry();
  geometry.setAttribute("position", new THREE.BufferAttribute(new Float32Array([
    -1,
    -1,
  ...
  writeSandstormBand({ rng, offset, shape, start: 0, count: SANDSTORM_NEAR_COUNT, area: 28, yMin: -3, yMax: 0.85, speedMin: 1.1, speedMax: 3, sizeMin: 0.058, sizeMax: 0.21, opacityMin: 0.05, opacityMax: 0.18 });
...
function writeSandstormBand(options) {
...
function createSplashGeometry(count) {
  const geometry = new THREE.InstancedBufferGeometry();
  geometry.setAttribute("position", new THREE.BufferAttribute(new Float32Array([
    -1,
    -1,
