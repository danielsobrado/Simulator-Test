...
function createRibbonGeometry(maxSegments) {
...
function writeVertex(array, offset, point) {
  array[offset] = point.x;
  array[offset + 1] = point.y;
  array[offset + 2] = point.z;
  return offset + 3;
}
function writeUv(array, offset, x, y) {
  array[offset] = x;
  array[offset + 1] = y;
  return offset + 2;
}
function resolveRibbonSide(point, tangent, cameraPosition, width, target) {
...
function writeRibbonGeometry(handle, polylines, cameraPosition, width, endWidthRatio) {
  const array = handle.position.array;
  const uvArray = handle.uv.array;
...
function createPointBuffer(segmentCount) {
  return Array.from({ length: Math.max(1, segmentCount) + 1 }, () => new THREE.Vector3());
}
function writeLightningArcPoints(points, start, end, jitter, seed) {
  ...
  const valueNoise = (t, frequency, noiseSeed) => {
    const x = t * frequency;
  ...
  const fractalNoise = (t, noiseSeed) => {
    let value = 0;
    let amplitude = 1;
    let normalization = 0;
  ...
  for (let i = 0; i <= span; i++) {
    ...
    const sideNoise = (fractalNoise(t, seed + 1.7) * 0.9 + (hash01(i, seed + 31.9) * 2 - 1) * 0.1) * jitterScale * envelope;
    const upNoise = (fractalNoise(t, seed + 9.3) * 0.9 + (hash01(i, seed + 47.1) * 2 - 1) * 0.1) * jitterScale * envelope;
    point.addScaledVector(side, sideNoise).addScaledVector(up, upNoise);
  }
}
function generateLightningArcPoints(start, end, segmentCount, jitter, seed) {
...
function computeLightningSpellFrame(startMs, durationMs, nowMs) {
  const elapsed = nowMs - startMs;
  const progress = elapsed / Math.max(1, durationMs);
  return { active: progress < 1, progress, timeSeconds: elapsed / 1e3 };
}
function computeLightningEnvelope(progress) {
  const p = clamp01(progress);
  const pulse = (start, attackEnd, fadeStart, end, intensity) => smoothstep(start, attackEnd, p) * (1 - smoothstep(fadeStart, end, p)) * intensity;
  return Math.max(
    pulse(0, 6e-3, 0.055, 0.16, 1),
    pulse(0.17, 0.19, 0.27, 0.34, 0.82),
    pulse(0.37, 0.39, 0.46, 0.56, 0.48)
  );
}
function makeGroundQuaternion(normal) {
  return new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 0, 1), normal.clone().normalize());
}
function makeSparkStates(count) {
...
function createLightningSpellVfx(deps) {
  ...
  const updateBranches = (seed) => {
    ...
    for (let index = 0; index < branchPoints.length; index++) {
      ...
      branchDirection.multiplyScalar(0.45).addScaledVector(branchSide, hash01(index, seed + 53) * 2 - 1).addScaledVector(branchUp, hash01(index, seed + 67) * 1.6 - 0.35).normalize();
  ...
  const updateSparks = (timeSeconds, envelope) => {
    ...
    for (let index = 0; index < sparkStates.length; index++) {
      ...
      sparkPosition.copy(state.target).addScaledVector(spark.direction, radialDistance).addScaledVector(state.targetNormal, IMPACT_SURFACE_BIAS + lift);
      const size = spark.scale * (1 - age) * envelope;
      sparkScale.setScalar(Math.max(1e-3, size));
  ...
  const updateActiveFrame = (frameNow) => {
