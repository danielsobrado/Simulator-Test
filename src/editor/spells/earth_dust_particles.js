...
function createParticleMaterial() {
  const fragment = Fn(() => {
    ...
    const disc = float(1).sub(smoothstep(0.42, 1, r));
    const core = float(1).sub(smoothstep(0, 0.26, r));
    const edgeBreakup = float(1).sub(smoothstep(0.22, 0.92, r)).mul(0.55).add(0.28);
    const alpha = clamp(disc.mul(edgeBreakup).add(core.mul(0.12)), 0, 0.42);
    const darkDust = vec3(0.34, 0.24, 0.15);
    const warmDust = vec3(0.66, 0.52, 0.36);
    const color = mix(darkDust, warmDust, core.mul(0.25).add(disc.mul(0.55)));
...
function resetInstance(mesh, index, matrix) {
  matrix.compose(new THREE.Vector3(0, -1e5, 0), new THREE.Quaternion(), ZERO_SCALE);
  mesh.setMatrixAt(index, matrix);
}
function createEarthDustParticleSystem(deps) {
  ...
  const buildSeeds = () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const startRadius = deps.config.impactRadius * (0.1 + rndB * 0.38);
      ...
      seeds.push({
        ...
        sizeM: deps.config.dustRadius * (0.06 + rndA * 0.095),
        spinRps: -2.8 + rndC * 5.6,
        phase: rndA * Math.PI * 2
      });
    }
  };
  const setParticleMatrix = (index, seed, ageS, lifeT, timeSeconds) => {
    ...
    position.copy(seed.origin).addScaledVector(seed.velocity, dragTravel).addScaledVector(normal, gravityFall).addScaledVector(seed.sideDrift, turbulence);
    const fade = smoothstepCpu(0, 0.12, lifeT) * (1 - smoothstepCpu(0.58, 1, lifeT));
    const grow = 0.48 + smoothstepCpu(0, 0.65, lifeT) * 1.75;
    const size = seed.sizeM * grow * Math.max(1e-3, fade);
