...
function computeEarthSpellFrame(startMs, durationMs, nowMs) {
  const elapsed = nowMs - startMs;
  const progress = elapsed / Math.max(1, durationMs);
  return { active: progress < 1, progress, timeSeconds: elapsed / 1e3 };
}
function computeEarthLightEnvelope(progress) {
  const p = clamp01(progress);
  return smoothstep(0, 0.12, p) * (1 - smoothstep(0.62, 1, p));
}
function spellColor(color) {
  return new THREE.Color(color[0], color[1], color[2]);
}
function makeGroundQuat(normal) {
...
function makeShardStates(config) {
...
function makeDustLayers(scene, config, geometry) {
  const count = Math.min(DUST_LAYER_MAX_COUNT, Math.max(DUST_LAYER_MIN_COUNT, Math.round(config.dustRadius * DUST_LAYER_DENSITY)));
  return Array.from({ length: count }, (_, i) => {
    const rndA = seeded01(i, 1);
    const rndB = seeded01(i, 2);
    const rndC = seeded01(i, 3);
    const handle = createEarthDustNodeMaterial({ seed: i * 17.17 + 3.1, opacity: 0.4 + rndC * 0.24 });
    ...
    return {
      ...
      startRadius: config.impactRadius * (0.15 + rndA * 0.2),
      endRadius: config.dustRadius * (0.55 + rndB * 0.5),
      rise: 0.75 + rndC * 1.85,
...
function createEarthSpellVfx(deps) {
  const { scene, config } = deps;
  const now = deps.now ?? (() => performance.now());
  const materialHandle = createEarthNodeMaterial();
  ...
  const updateDust = (progress, timeSeconds) => {
    const dustProgress = clamp01(progress * 1.18);
    const travel = smoothstep(0, 0.78, dustProgress);
    const fade = 1 - smoothstep(0.6, 1, dustProgress);
    const lift = smoothstep(0, 0.58, dustProgress);
    for (const dust of dustLayers) {
      ...
      dust.mesh.rotation.set(0.04 * Math.sin(timeSeconds + dust.angle), dust.yaw + timeSeconds * 0.18, 0.1 * Math.cos(timeSeconds * 0.8 + dust.angle));
  ...
  const updateShards = (progress) => {
    ...
    const travel = smoothstep(0, 0.86, shardProgress);
    const fade = 1 - smoothstep(0.68, 1, shardProgress);
...
export {
  computeEarthLightEnvelope,
  computeEarthSpellFrame,
  createEarthSpellVfx
};
