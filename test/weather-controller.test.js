...
} from '../src/editor/weather/meadow.js';
...
test('explicit meadow mode retains its visibility boost', () => {
  const environment = resolveMeadowEnvironment(
    { weatherMode: 'meadow' },
    { amount: 0.1, coldBlend: 0.2, localMist: 0.3 },
    new THREE.Vector3(),
    new THREE.Vector3(0, 1, 0),
  );

  assert.equal(environment.amount, Math.max(MEADOW_MODE_VISUAL_AMOUNT, 0.1));
  assert.equal(environment.visibilityGain, MEADOW_MODE_VISIBILITY_GAIN);
});
